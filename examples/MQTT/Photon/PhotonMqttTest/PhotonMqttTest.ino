#include "MQTT.h"

#define SERVER "broker.shiftr.io"
#define PORT 1883

#define USERNAME "try"
#define PASSWORD "try"

#define DEVICE_NAME "Waldemar's Photon"
#define SUBSCRIBE_CHANNEL "waldemar/to_photon"
#define PUBLISH_CHANNEL "waldemar/from_photon"

void callback(char* topic, byte* payload, unsigned int length);

/**
 * if want to use IP address,
 * byte server[] = { XXX,XXX,XXX,XXX };
 * MQTT client(server, 1883, callback);
 * want to use domain name,
 * MQTT client("www.sample.com", 1883, callback);
 **/
MQTT client(SERVER, PORT, callback);

// recieve message
void callback(char* topic, byte* payload, unsigned int length) {
    char p[length + 1];
    memcpy(p, payload, length);
    p[length] = NULL;
    String message(p);

    // check if message contains a color, and light up LED accordingly
    if (message.equals("RED"))
        RGB.color(255, 0, 0);
    else if (message.equals("GREEN"))
        RGB.color(0, 255, 0);
    else if (message.equals("BLUE"))
        RGB.color(0, 0, 255);
    else
        RGB.color(255, 255, 255);
    delay(1000);
}


void setup() {
    Serial.begin(9600);
    RGB.control(true);

    // connect to the server
    client.connect("tsp_photon", "try", "try");

    // publish/subscribe
    if (client.isConnected()) {
        client.publish(PUBLISH_CHANNEL, "Hi from Photon! ");
        client.subscribe(SUBSCRIBE_CHANNEL);
    }
}

void loop() {
    if (client.isConnected()){
      client.loop();
    }
}
